...
void self_and_line(void *arg)
{
    ...
    while (1)
    {
        ...
        left_combined_cycle = bound(motor_cmd , MIN_PWM, MAX_PWM); // scaling factor
        right_combined_cycle = bound(motor_cmd , MIN_PWM, MAX_PWM);
        x = read_pid_const2().x;
        y = read_pid_const2().y;
        ...
        else
        {
            ...
            //  left_duty_cycle = bound((optimum_duty_cycle - correction), lower_duty_cycle, higher_duty_cycle);
            // right_duty_cycle = bound((optimum_duty_cycle + correction), lower_duty_cycle, higher_duty_cycle);
            left_duty_cycle = bound((forward_pwm - correction), lower_duty_cycle, higher_duty_cycle); // forward pwm  replace with motorcmd
