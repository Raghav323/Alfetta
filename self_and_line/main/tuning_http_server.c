...
static pid_const_t pid_constants = {.kp = 8, .ki = 0, .kd = 3.3, .val_changed = true};
static pid_const2_t pid_constants2 = {.kp2 = 5.8, .ki2 = 0, .kd2 = 1.6, .setpoint =2, .offset = 0.0,.x=0.5,.y=-0.5,.errorf=5,.hlp=66.125 ,.val_changed = true}; //random values for now.
...
static esp_err_t tuning_pid_post_handler(httpd_req_t *req)
{
    ...
    pid_constants2.val_changed = true;
    return ESP_OK;
}
...
pid_const2_t read_pid_const2()
{
    return pid_constants2;
}
...
void reset_val_changed_pid_const2()
{
    pid_constants2.val_changed = false;
}
